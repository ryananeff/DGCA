...
author: "Andrew McKenzie, Ryan Neff, Bin Zhang"
...
Next, we will create a cluster of R threads using doParallel for parallel DGCA analysis, which offers a significant speedup on multi-threaded machines and large datasets. 

```{r, results="hide", warning=FALSE, message=FALSE}
library(doParallel)
no_cores <- 8 #set this to the number of cores that you have available
...
ddcor_res = ddcorAll(inputMat = darmanis, design = design_mat,
  ...
  adjust = "none", nPerm = 0, nPairs = Inf, cl=cl)
head(ddcor_res)
```
...
ddcor_res = ddcorAll(inputMat = darmanis, design = design_mat,
  compare = c("oligodendrocyte", "neuron"),
  adjust = "perm", nPerm = 5, splitSet = "RTN4", nPairs=Inf, cl=cl)
head(ddcor_res)
```
